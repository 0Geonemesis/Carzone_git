...
public class FrmReporteEstadisticasVentas extends JFrame {
    ...
    private static final Color C_NARANJA = new Color(170, 90, 15);
    private static final Color C_MORADO = new Color(90, 40, 130);
    ...
    private JLabel lblCantidad, lblMaximo, lblMinimo, lblPromedio;

    private List<Venta> ventasActuales;
    private String rangoDescripcionActual = "Todas las fechas";

    public FrmReporteEstadisticasVentas(Usuario usuarioActual) {
    ...
    private void initComponents() {
        setTitle("CarZone — Reporte de Estadisticas de Ventas");
        setSize(1080, 680);
    ...
    private JPanel crearHeader() {
        ...
        JLabel lblTitulo = new JLabel("CARZONE  |  Reporte de Estadisticas de Ventas");
    ...
    private JPanel crearPanelResumen() {
        ...
        lblCantidad = crearTarjetaMetrica("0");
        lblMaximo = crearTarjetaMetrica("S/. 0.00");
        lblMinimo = crearTarjetaMetrica("S/. 0.00");
        lblPromedio = crearTarjetaMetrica("S/. 0.00");

        p.add(contenedorTarjeta("Ventas con monto", lblCantidad, C_AZUL));
        p.add(contenedorTarjeta("Monto maximo", lblMaximo, C_VERDE));
        p.add(contenedorTarjeta("Monto minimo", lblMinimo, C_NARANJA));
        p.add(contenedorTarjeta("Promedio de ventas", lblPromedio, C_MORADO));
        return p;
    }
    ...
    private JPanel contenedorTarjeta(String titulo, JLabel lblValor, Color color) {
        ...
        lblTit.setForeground(new Color(220, 230, 255));
    ...
    private List<Venta> filtrarSoloCompletadas(List<Venta> lista) {
        List<Venta> resultado = new java.util.ArrayList<>();
        if (lista == null) {
            return resultado;
        }
        for (Venta v : lista) {
            if ("completada".equalsIgnoreCase(v.getEstadoVenta())) {
                resultado.add(v);
            }
        }
        return resultado;
    ...
    private void refrescarTabla() {
        ...
        for (Venta v : filtrarSoloCompletadas(ventasActuales)) {
        ...
        lblCantidad.setText(String.valueOf(cantidadConMonto));
        lblMaximo.setText(String.format("S/. %,.2f", maximo != null ? maximo : BigDecimal.ZERO));
        lblMinimo.setText(String.format("S/. %,.2f", minimo != null ? minimo : BigDecimal.ZERO));
        lblPromedio.setText(String.format("S/. %,.2f", promedio));
    }
    ...
    private void descargarPDF() {
        ...
        chooser.setDialogTitle("Guardar Reporte de Estadisticas de Ventas");
        String nombreSugerido = "ReporteEstadisticasVentas_" + new SimpleDateFormat("yyyyMMdd_HHmmss").format(new java.util.Date()) + ".pdf";
        ...
        try {
            Reporte reporte = ReporteFactory.crear(
                    ReporteFactory.TIPO_ESTADISTICAS_VENTAS,
                    filtrarSoloCompletadas(ventasActuales),
